...
import com.messmanagement.subscription.dto.MySubscriptionStatusDTO;
...
public class SubscriptionController {
    ...
    @PreAuthorize("hasRole('STUDENT')")
    public ResponseEntity<MySubscriptionStatusDTO> getMySubscriptionStatus(Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String userEmail = authentication.getName(); // Get authenticated user's email
        MySubscriptionStatusDTO statusDto = subscriptionService.getDetailedUserSubscriptionStatus(userEmail);
